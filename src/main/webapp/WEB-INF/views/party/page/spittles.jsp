...
<input type="button" data-role="button" id="refreshSpittles"  value="刷新" />
...
<ul data-role="listview" data-split-theme="d" data-inset="true" data-count-theme="b" id="spittleList">
    ...
    <li class="spittle" data-spittleId="${spittle.id}">
       <h2>${user.id==1?spittle.id:''}${user.id==1?'.':''}${spittle.nickname}</h2>
        <p>${spittle.content}</p>
        <span class="ui-li-count spittle-likes" style="right:50px;">赞  ${spittle.likes}</span>
        <span class="ui-li-count spittle-dislikes">踩  ${spittle.dislikes}</span>
...
<input type="button" data-role="button" id="moreSpittles"  value="更多" />
	...
	var dislikeClick = function() {
		...
		$.isst.api.dislike(spittleId, function(response) {
			if (response.code > 0) {
				...
				$spittle.find('.spittle-dislikes').text(parseInt($spittle.find('.spittle-dislikes').text())+1);
			} else {
	...
	var  likeClick = function() {
		...
		$.isst.api.like(spittleId, function(response) {
			if (response.code > 0) {
				$this.buttonMarkup({theme: 'b'}).button('refresh');
				$spittle.find('.spittle-like, .spittle-dislike').button('disable');
				$spittle.find('.spittle-likes').text(parseInt($spittle.find('.spittle-likes').text())+1);
			} else {
				alert(response.message);
			}
		});
